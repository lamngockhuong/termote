...
- **Switch**: Click session in left sidebar or in the **session tabs** at the top (if enabled)
...
- **Session tabs**: Horizontal tab bar for quick session switching on desktop. Can be toggled via Settings → "Show session tabs"
...
| Auto-sync           | Sessions sync across browsers (configurable)     |
| Persistence         | Sessions backed by tmux windows                  |
| Connection status   | Real-time indicator in header (see below)        |

## Connection Indicator

The header displays a real-time connection status indicator showing your link to the server:

| Status        | Indicator | Meaning                                     |
| ------------- | --------- | ------------------------------------------- |
| Connected     | Green dot | Server online, terminal responsive           |
| Connecting    | Blue dot  | Establishing connection to server            |
| Disconnected  | Red dot   | No connection to server (clickable to retry) |
| Error         | Red X     | Connection error occurred                   |

The indicator auto-detects disconnects via session polling. Click it when disconnected to manually retry the connection.
