...
- **Chuyển**: Click vào phiên trong sidebar bên trái hoặc trong **thanh tab phiên** ở trên cùng (nếu bật)
...
- **Thanh tab phiên**: Thanh tab ngang để chuyển phiên nhanh trên desktop. Bật/tắt qua Settings → "Show session tabs"
...
| Tự động đồng bộ   | Phiên đồng bộ giữa các trình duyệt (tùy chỉnh được) |
| Lưu trữ           | Phiên được lưu bởi tmux windows               |
| Trạng thái kết nối | Chỉ báo thời gian thực trên header (xem bên dưới) |

## Chỉ báo kết nối

Header hiển thị chỉ báo trạng thái kết nối thời gian thực với server:

| Trạng thái   | Chỉ báo    | Ý nghĩa                                    |
| ------------ | ---------- | ------------------------------------------- |
| Connected    | Chấm xanh  | Server hoạt động, terminal phản hồi         |
| Connecting   | Chấm xanh dương | Đang kết nối đến server                  |
| Disconnected | Chấm đỏ    | Mất kết nối (nhấn để thử lại)               |
| Error        | X đỏ       | Lỗi kết nối                                 |

Chỉ báo tự động phát hiện mất kết nối qua session polling. Nhấn vào khi mất kết nối để thử lại kết nối thủ công.
