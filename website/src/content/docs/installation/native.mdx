...
### tmux Configuration (Recommended)

For tmux copy/paste to work with the keyboard toolbar buttons, add this to your `~/.tmux.conf`:

```bash
# Vi mode for copy: Space=start selection, Enter=copy
set-option -g mode-keys vi
# Mouse support (scroll, select) - toggle with Ctrl+b m
...
This enables: **Copy icon** → navigate → **Space** → select → **Enter** → **Paste icon**

:::tip[Copy to System Clipboard]
With mouse ON, tmux captures mouse events. To copy to system clipboard:
- **Hold Shift** while selecting text (bypasses tmux, uses terminal selection)
- Or press **Ctrl+b m** to toggle mouse off temporarily
:::
...
2. Run deploy command
   ...
   <Tabs>
     ...
     <TabItem label="Windows (PowerShell)">
       ...
       By default (`-Ttyd fork`), the installer downloads the
       [fork/MSVC ttyd build](https://github.com/lamngockhuong/ttyd/releases)
       (`ttyd.msvc.exe`), since the upstream `tsl0922/ttyd` Windows build
       (`ttyd.win32.exe`) is currently broken on the latest Windows. Pass
       `-Ttyd official` to use the upstream build instead.
     </TabItem>
   </Tabs>
...
<Tabs>
  ...
  <TabItem label="Windows (PowerShell)">
    Native mode writes service logs to `~/.termote/logs`. View them with the CLI:

    ```powershell
    .\scripts\termote.ps1 logs            # Show recent ttyd + tmux-api logs
    .\scripts\termote.ps1 logs ttyd       # Only ttyd logs
    .\scripts\termote.ps1 logs tmux-api   # Only tmux-api logs
...
| PWA not built   | `pnpm --filter termote build`                            |
...
If Termote is already installed, use the built-in update command:

<Tabs>
  <TabItem label="Unix (macOS/Linux)">
    ```bash
    termote update                   # Update to latest version
    termote update --version 0.0.9   # Pin to specific version
    termote update --force           # Force reinstall current version
    ```
  </TabItem>
  <TabItem label="Windows (PowerShell)">
    ```powershell
    .\scripts\termote.ps1 update                 # Update to latest version
    .\scripts\termote.ps1 update -Version 0.0.9  # Pin to specific version
    .\scripts\termote.ps1 update -Force          # Force reinstall current version
    ```
  </TabItem>
</Tabs>

This automatically preserves your config (including your chosen `ttyd` build on Windows), stops services, downloads the new version, verifies checksums, and re-installs with your saved settings.
